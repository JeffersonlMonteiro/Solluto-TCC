...
router.post('/forgot-password', async (req, res) => {
  ...
  usuario.USU_TXT_IMGPATH = token; // Reusando campo para token (ou adicione novo)
  usuario.USU_TMS_DTALT = new Date(Date.now() + 3600000); // Expira em 1h
  await usuario.save();
...
router.post('/reset-password', async (req, res) => {
  ...
  const usuario = await Usuario.findOne({
    where: { USU_TXT_IMGPATH: token, USU_TMS_DTALT: { [Op.gt]: new Date() } }
  });
  ...
  usuario.USU_TXT_IMGPATH = null;
  usuario.USU_TMS_DTALT = new Date();
  await usuario.save();
